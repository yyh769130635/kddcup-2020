...
def get_sim_item(df_, user_col, item_col, use_iif=False): 
            ...
            for loc2, relate_item in enumerate(items):  
                ...
                if not use_iif:  
                    if loc1-loc2>0:#比较点击的前后顺序，隔的远，相关性低，0.8可调，log消除用户活跃度影响
                        sim_item[item][relate_item] += 1 * 0.7 * (0.88**(loc1-loc2-1)) * (1 - (t1 - t2) * 10000) / math.log(1 + len(items)) # 逆向
                    else:
                        sim_item[item][relate_item] += 1 * 1.0 * (0.88**(loc2-loc1-1)) * (1 - (t2 - t1) * 10000) / math.log(1 + len(items)) # 正向
                else:  
    ...
    for i, related_items in tqdm(sim_item.items()):  
        for j, cij in related_items.items():  
            sim_item_corr[i][j] = cij / ((item_cnt[i] * item_cnt[j]) ** 0.19)  #这里要改一下，解决哈利波特，商品热门的影响
...
for c in range(now_phase + 1):  
    ...
    for i in tqdm(click_test['user_id'].unique()):
        ...
        rank_item = recommend(item_sim_list, user_item, i, 13000, 13000)
        for j in rank_item:
...
result.to_csv('./result_real/baseline-5-5-4.csv', index=False, header=None)