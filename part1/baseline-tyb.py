        ...
        for j, wij in sorted(sim_item_corr[i].items(), reverse=True):#[0:top_k]:  
            if j not in interacted_items: 
                ...
                if j in tiaozheng_list:
                    ...
                    if(temp[str_time-1]<=0.125):
                        rank[j] += wij * (0.7 ** loc)
                    else:
                        rank[j] += wij * (0.7 ** loc)*np.log2(40*temp[str_time - 1])
                    # rank[j] += wij * (0.7 ** loc) * np.log2(1.4+30* temp[str_time-1])
                    # rank[j] += wij * (0.7 ** loc) *8* temp[str_time - 1]
...
if __name__ == "__main__":
    ...
    result.to_csv('baseline-(5-1)-3.csv', index=False, header=None)
    end = time.time()
